  ...
  useEffect(() => {
    async function getTransactionForMonth({transaction, date}) {
        try {
          ...
          const necessaryData = data.map(({_id, date, description, category, amount}) => ({_id, date: new Date(date), description, category, amount})).reverse();
  ...
  return (
    <TableContainer className={styled.wrapper}>
      <Table size="small" className={styled.table}>
        ...
        <TableBody>
          {transactionList.map(({_id, date, description, category, amount}) => (
            <TableRow key={_id}>
              ...
              <TableCell>
                <button type="button" className={styled.trash} onClick={() => onDelete(_id)}>
                  <IoTrashOutline />
                </button>
              </TableCell>
            </TableRow>
